            ...
            <Dialog open={openTransfer} onOpenChange={setOpenTransfer}>
                <DialogContent><DialogHeader><DialogTitle>Transferencia Suma Cero</DialogTitle></DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div><Label>Origen (Resta)</Label><Select value={formTransfer.from} onValueChange={v=>setFormTransfer(f=>({...f,from:v}))}><SelectTrigger><SelectValue placeholder="Cuenta origen" /></SelectTrigger><SelectContent>{accounts.map(a=><SelectItem key={a.id} value={a.id}>{a.name} ({formatCOP(a.balance)})</SelectItem>)}</SelectContent></Select></div>
                        <div><Label>Destino (Suma)</Label><Select value={formTransfer.to} onValueChange={v=>setFormTransfer(f=>({...f,to:v}))}><SelectTrigger><SelectValue placeholder="Cuenta destino" /></SelectTrigger><SelectContent>{accounts.map(a=><SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent></Select></div>
